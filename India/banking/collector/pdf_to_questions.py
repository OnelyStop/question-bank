"""
Convert bank exam PYQ PDFs in corpus/ into structured JSON under question_bank/.
...
DEFAULT_OUT = ROOT / "question_bank"
...
OPTION_SPLIT_RE = re.compile(r"(?i)(?:\(\s*([a-e])\s*\)|([a-e])\s*\))\s+")
...
def parse_questions_from_text(
    ...
    while i < len(events):
        ...
        if ev["kind"] == "direction":
            ...
            # Direction body runs until first applicable question (or next direction)
            body_end = len(text)
            ...
            dir_preamble, shared_opts = parse_options(raw_body)
            direction_text = normalize_ws(dir_preamble) if dir_preamble else normalize_ws(raw_body)
            if len(direction_text) < 20 and shared_opts:
        ...
        # question
        q_num = ev["q_num"]
        # Skip likely false positives (page debris): numbers way above 200
        if q_num > 200:
            i += 1
            continue
        if q_num in seen_nums:
            # Duplicate numbering (e.g. repeated header) — skip
            i += 1
            continue

        # Clear ranged direction if we're past its range (bare dirs stay until replaced)
        if active_dir and active_dir.get("q_start") is not None:
            if not direction_covers(active_dir, q_num):
                active_dir = None
        ...
        for j in range(i + 1, len(events)):
            ...
            if nxt["kind"] == "direction":
                ...
                break
            if nxt["kind"] == "question":
                # Next different question number ends this block
                if nxt["q_num"] != q_num:
                    block_end = nxt["start"]
                    break
        ...
        # Inherit shared options from direction when question has none
        ...
        # Cloze / blank items often are options-only under a passage direction
        ...
        # Very short stems with no options are usually noise
...
def convert_pdf(
    ...
    try:
        ...
        text, page_spans = build_full_text(pages)
        questions, notes = parse_questions_from_text(text, page_spans, paper_id)
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(description="Convert corpus PDFs to question JSON")
    ...
    for idx, pdf in enumerate(pdfs, start=1):
        ...
        result = convert_pdf(pdf, corpus, out_root, force=args.force)
        # Drop heavy objects before storing in report list
        slim = {k: v for k, v in result.items() if k not in {"questions", "paper"}}
        results.append(slim)
